...
let moverEvent = function (e) {
  ...
  let emojiMaxDisplacementX = (relX / wrapperRect.width) * emojiFaceMaxDisplacement
  let emojiMaxDisplacementY = (relY / wrapperRect.height) * emojiFaceMaxDisplacement
  gsap.to(emojiFace, {
...
gsap.to("[data-direct]", { //속성중에 data-direct이 있는 것들을 모두 불러줌(호출)
...
gsap.to('[data-speed]', {
  ...
  ease: 'none',
  scrollTrigger: {
    start: 0, //스크롤 트리거가 시작되는 스크롤 위치
    end: 'max', //스크롤 트리거가 끝나는 위치되는 스크롤 위치
    scrub: 2,
  }
})
...
//wrapper.addEventListener('mousemove',function(){moverEvent()})
wrapper.addEventListener('mousemove', moverEvent)