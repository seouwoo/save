...
email.addEventListener('focusout',inEmailCorrect)
...
function comparePw(){
    if(pw1.value === pw2.value && pw2.value !== ""){//value안에 아무것도 없다면?
        pwImg2.src="./img/m_icon_check_enable.png"//이미지로 변경
        error[2].style.display='none'

    }else if(pw1.value !== pw2.value){//각자 value값이 다르면?
        pwImg2.src="./img/m_icon_check_enable.png"//이미지로 변경
...
function isBirthCompleted(){
    ...
    function inMonthCompleted(){
        if(mm.value ==- '월'){
            error[4].style.display='block';
            error[4].innerHTML='태어난 년도 월을 선택하세요.'
        }else{
    ...
    function isBirthRigth(){
        let datePatten=/\d{1,2}/;

        if (! datePatten.test(dd.value) || Number(dd.value)>1 ||  Number(dd.value)>31){
            error[4].style.display='block';
...
function checkphoneNum(){
    // 010 -0000-0000
    let isphoneNum=/^([0-1]{2})([01679]{1})([0-9]{3,4})([0-9]{4})$/
    if(mobile.value === ''){
