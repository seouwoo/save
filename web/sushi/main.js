...
let scrollActive=()=>{
    let scrollYY=pageYOffset
    //let scrollYY=scrollY
    //console.log(scrollYY)
    // let sections=document.querySelectorAll('section[id]')//section태그중 속성값이 id가 있는것
    let sections=document.querySelectorAll('.section[id]')
    sections.forEach((current)=>{
        let sectionHeight=current.offsetHeight;//각 섹션의 높이값
        let sectionTop=current.offsetTop - 80;//각 섹션의 전체문서에서의 top위치
